...
if __name__ == '__main__':
    ...
    # Creates ReviewCorpus object using the json file of yelp reviews
    reviewCorp = ReviewCorpus('yelp_reviews.json')
    # Takes the content of attribute 'datafiles' (which is the filename of 'yelp reviews json' and loads it so that
    # all the reviews with their labels are stored in the 'documents' attribute of 'reviewCorp'
    reviewCorp.load(reviewCorp.datafiles[0], document_class=Document)
    print(reviewCorp.documents[0].data)
    print(reviewCorp.documents[0].label)
    print(reviewCorp.documents[0].source)
    print(reviewCorp.documents[0].feature_vector)
    print(reviewCorp.documents[0].features())

    namesCorp = NamesCorpus()
    namesCorp.load(namesCorp.datafiles[0], document_class=Document)
    print(namesCorp.documents[0].data)